...
window.addEventListener('load',function(){
  ...
  var MAP=[['eb','.in-hero .eyebrow'],['lead','.in-hero .lead'],['cta1','.in-hero .btn-primary'],['cta2','.in-hero .btn-ghost'],['sub.eb','#in-sub-head .eyebrow'],['sub.h2','#in-sub-head .heading'],['flow.eb','#in-flow-head .eyebrow'],['flow.h2','#in-flow-head .heading'],['out.eb','#in-out-head .eyebrow'],['out.h2','#in-out-head .heading'],['end.eb','.in-cta .eyebrow'],['end.h2','.in-cta .heading'],['end.p','.in-cta p'],['end.b2','.in-cta .btn-ghost']];
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    gsap.fromTo('.in-cta',{opacity:0,y:30,scale:.98},{opacity:1,y:0,scale:1,duration:.7,ease:'power3.out',scrollTrigger:{trigger:'.in-cta',start:'top 88%',once:true}});
    setTimeout(function(){try{ScrollTrigger.refresh();}catch(e){}},400);
  } else {
    ['.in-sub','.in-out','.in-cta','.in-node'].forEach(function(sel){d.querySelectorAll(sel).forEach(function(el){el.style.opacity='1';el.style.transform='none';});});
  }
