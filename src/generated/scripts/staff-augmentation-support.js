...
window.addEventListener('load',function(){
  ...
  var MAP=[['eb','.tl-hero .eyebrow'],['lead','.tl-hero .lead'],['cta1','.tl-hero .btn-primary'],['cta2','.tl-hero .btn-ghost'],['sub.eb','#tl-sub-head .eyebrow'],['sub.h2','#tl-sub-head .heading'],['flow.eb','#tl-flow-head .eyebrow'],['flow.h2','#tl-flow-head .heading'],['out.eb','#tl-out-head .eyebrow'],['out.h2','#tl-out-head .heading'],['end.eb','.tl-cta .eyebrow'],['end.h2','.tl-cta .heading'],['end.p','.tl-cta p'],['end.b2','.tl-cta .btn-ghost']];
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    gsap.fromTo('.tl-cta',{opacity:0,y:30,scale:.98},{opacity:1,y:0,scale:1,duration:.7,ease:'power3.out',scrollTrigger:{trigger:'.tl-cta',start:'top 88%',once:true}});
    setTimeout(function(){try{ScrollTrigger.refresh();}catch(e){}},400);
  } else {
    ['.tl-model','.tl-sub','.tl-out','.tl-cta','.tl-card'].forEach(function(sel){d.querySelectorAll(sel).forEach(function(el){el.style.opacity='1';el.style.transform='none';});});
  }
