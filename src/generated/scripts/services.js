...
window.addEventListener('load',function(){
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    d.querySelectorAll('.ind-card').forEach(function(c,i){gsap.from(c,{opacity:0,y:26,duration:.5,delay:(i%4)*.09,ease:'power3.out',scrollTrigger:{trigger:c,start:'top 88%',once:true}});});
