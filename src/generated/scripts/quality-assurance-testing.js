...
window.addEventListener('load',function(){
  ...
  var MAP=[['eb','.qa-hero .eyebrow'],['lead','.qa-hero .lead'],['cta1','.qa-hero .btn-primary'],['cta2','.qa-hero .btn-ghost'],['pyr.eb','#qa-pyr-head .eyebrow'],['pyr.h2','#qa-pyr-head .heading'],['out.eb','#qa-out-head .eyebrow'],['out.h2','#qa-out-head .heading'],['sub.eb','#qa-sub-head .eyebrow'],['sub.h2','#qa-sub-head .heading'],['end.eb','.qa-cta .eyebrow'],['end.h2','.qa-cta .heading'],['end.p','.qa-cta p'],['end.b2','.qa-cta .btn-ghost']];
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    gsap.fromTo('.qa-cta',{opacity:0,y:30,scale:.98},{opacity:1,y:0,scale:1,duration:.7,ease:'power3.out',scrollTrigger:{trigger:'.qa-cta',start:'top 88%',once:true}});
    setTimeout(function(){try{ScrollTrigger.refresh();}catch(e){}},400);
  } else {
    ['.qa-gate','.qa-tier','.qa-out','.qa-sub','.qa-cta','.qa-term'].forEach(function(sel){d.querySelectorAll(sel).forEach(function(el){el.style.opacity='1';el.style.transform='none';});});
  }
