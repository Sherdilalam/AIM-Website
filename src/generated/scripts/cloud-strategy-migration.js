...
window.addEventListener('load',function(){
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    d.querySelectorAll('.sa-path').forEach(function(p,i){gsap.from(p,{opacity:0,y:24,duration:.5,delay:(i%3)*.1,ease:'power3.out',scrollTrigger:{trigger:p,start:'top 88%',once:true}});});
    d.querySelectorAll('.sa-step').forEach(function(s,i){gsap.from(s,{opacity:0,y:24,duration:.5,delay:(i%3)*.09,ease:'power3.out',scrollTrigger:{trigger:s,start:'top 90%',once:true}});});
  ...
  bind(d.querySelectorAll('.sa-pillar,.sa-why-card,.sa-path,.sa-step,a'));
