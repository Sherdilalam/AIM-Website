...
window.addEventListener('load',function(){
  ...
  var MAP=[['eb','.da-hero .eyebrow'],['lead','.da-hero .lead'],['cta1','.da-hero .btn-primary'],['cta2','.da-hero .btn-ghost'],['tab.eb','#da-tab-head .eyebrow'],['tab.h2','#da-tab-head .heading'],['out.eb','#da-out-head .eyebrow'],['out.h2','#da-out-head .heading'],['sub.eb','#da-sub-head .eyebrow'],['sub.h2','#da-sub-head .heading'],['end.eb','.da-cta .eyebrow'],['end.h2','.da-cta .heading'],['end.p','.da-cta p'],['end.b2','.da-cta .btn-ghost']];
  ...
  if(window.gsap&&window.ScrollTrigger&&!rm){gsap.registerPlugin(ScrollTrigger);
    ...
    gsap.fromTo('.da-cta',{opacity:0,y:30,scale:.98},{opacity:1,y:0,scale:1,duration:.7,ease:'power3.out',scrollTrigger:{trigger:'.da-cta',start:'top 88%',once:true}});
    setTimeout(function(){try{ScrollTrigger.refresh();}catch(e){}},400);
  } else {
    ['.da-mstep','.da-out','.da-sub','.da-cap','.da-cta'].forEach(function(sel){d.querySelectorAll(sel).forEach(function(el){el.style.opacity='1';el.style.transform='none';});});
  }
