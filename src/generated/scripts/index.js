...
window.addEventListener('load', function(){
  ...
  var CASES={en:[
    ...
    {t:'GoRight Fleet AI Knowledge Assistant',tag:'Generative AI',d:'From 45+ minutes of manual search to verified answers in seconds - 1,000+ documents indexed, zero hallucinations, mandatory citations, live in production.',img:'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=900&q=80'}
  ],fr:[
    ...
    {t:'Assistant IA GoRight Fleet',tag:'IA g\u00e9n\u00e9rative',d:'De 45+ minutes de recherche manuelle \u00e0 des r\u00e9ponses v\u00e9rifi\u00e9es en secondes - 1 000+ documents index\u00e9s, z\u00e9ro hallucination.',img:'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=900&q=80'}
  ]};
  /* bilingual services */
  var SERV={en:[
  ...
  /* case studies */
  var CASE_SLUGS=['/it-digital-strategy','/bi','/itsm','/aim-enterprise-ai-solutions','/goright-fleet-ai-knowledge-assistant'];
  function renderCases(){var track=d.getElementById('csTrack');if(!track)return;track.innerHTML='';var L=getLang();var cases=CASES[L]||CASES.en;cases.forEach(function(c,ci){var card=d.createElement('article');card.className='cs-card';card.innerHTML='<div class="cs-media"><img decoding="async" src="'+c.img+'" alt="'+esc(c.t)+'" loading="lazy" onerror="this.style.display=\'none\'"></div><div class="cs-body"><span class="cs-tag">'+(L==='fr'?'\u00c9tude de cas':'Case Study')+' \u00b7 '+esc(c.tag)+'</span><h3>'+esc(c.t)+'</h3><p>'+esc(c.d)+'</p><a href="'+(CASE_SLUGS[ci]||'#')+'" class="cs-link">'+(L==='fr'?'Lire l\u2019\u00e9tude \u2192':'Read case study \u2192')+'</a></div>';track.appendChild(card);});bind(track.querySelectorAll('a,.cs-card'));function cardStep(){var f=track.querySelector('.cs-card');return f?f.getBoundingClientRect().width+24:380;}var nx=d.getElementById('csNext'),pv=d.getElementById('csPrev');var csHalted=false;function advance(){if(track.scrollLeft+track.clientWidth>=track.scrollWidth-8){track.scrollTo({left:0,behavior:'smooth'});}else{track.scrollBy({left:cardStep(),behavior:'smooth'});}}function pauseAuto(){if(window._csAuto){clearInterval(window._csAuto);window._csAuto=null;}}function startAuto(){if(rm||csHalted)return;pauseAuto();window._csAuto=setInterval(advance,4500);}
/* Once the visitor drives the slider themselves they have shown they know how
   to navigate it, so the timer stops for good instead of shifting the track
   out from under them mid-read. csHalted is what keeps mouseleave from
   restarting it; hover alone still only pauses, as before. */
function stopAuto(){csHalted=true;pauseAuto();}
if(nx){nx.onclick=function(){stopAuto();advance();};}if(pv){pv.onclick=function(){stopAuto();track.scrollBy({left:-cardStep(),behavior:'smooth'});};}
pauseAuto();startAuto();track.onmouseenter=pauseAuto;track.onmouseleave=startAuto;}
  renderCases();
  /* services explorer */
